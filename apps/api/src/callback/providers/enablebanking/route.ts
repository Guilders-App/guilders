...
const app = new Hono<{ Bindings: Bindings; Variables: Variables }>().get(
  ...
  async (c) => {
    ...
    if (!stateObj.userId || !stateObj.institutionId) {
      console.error(
        "Missing required state parameters: userId or institutionId",
      );
    ...
    const provider = await getProvider("EnableBanking", supabase, c.env);
    ...
    try {
      const enablebankingClient = new EnableBankingClient(
        c.env.ENABLEBANKING_CLIENT_ID,
        c.env.ENABLEBANKING_CLIENT_PRIVATE_KEY,
      );
